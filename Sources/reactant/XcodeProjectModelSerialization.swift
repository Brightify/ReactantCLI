...
extension Xcode.ResourcesBuildPhase: PropertyListSerializable {
    ...
    fileprivate func serialize(to serializer: PropertyListSerializer) -> [String: PropertyList] {
        // Create a `PBXResourcesBuildPhase` plist dictionary.
        // FIXME:  It would be nicer to be able to use inheritance for the code
        // inherited from BuildPhase, but but in Swift 3.0 we get an error that
        // "declarations in extensions cannot override yet".
        return makeBuildPhaseDict(buildPhase: self, serializer: serializer, xcodeClassName: "PBXResourcesBuildPhase")
    }
}
    ...
    func asPropertyList() -> PropertyList {
        ...
        for child in mirror.children {
            ...
            switch child.value {
              ...
              case let item as Xcode.BuildSettingsTable.BuildSettingsItem:
                if let value = item.value {
                    dict[name] = .string(value)
                }
                for (sdk, value) in item.sdkSpecificValues {
                    dict["\"\(name)[sdk=\(sdk.name)*]\""] = .string(value)
                }
              default:
